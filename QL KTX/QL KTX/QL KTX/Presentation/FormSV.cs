    ...
    class FormSV
    {
        ...
        public void them()
        {
            ...
            do
            {
                ...
                {
                    if (sv.Masv == lsv[i].Masv) 
                    {
                        gd.pain("DA CO MA SINH VIEN NAY,MOI KIEM TRA LAI",40,40);
                    }
                }
            ...
            do
            {
                gd.ve(9, 24); Console.WriteLine("Nhập địa chỉ"); gd.ve(50, 24); sv.Diachi = Console.ReadLine();
            ...
            do
            {
                ...
                Console.SetCursorPosition(50, 30); sv.Sdt1 = Console.ReadLine();
            }
            while (string.IsNullOrEmpty(sv.Sdt1));
            svbll.themsv(sv);
        svs:
        ...
        public void hien()
        {
            ...
            {
                gd.pain(Convert.ToString(p.Masv), 5, j);
                gd.pain(Convert.ToString(p.Maph), 13, j);
            ...
            gd.pain("MÃ SV", 3, 3); gd.pain("MÃ PHÒNG", 14, 3); gd.pain("HỌ TÊN", 30, 3); gd.pain("GIỚI TÍNH", 44, 3); gd.pain("ĐỊA CHỈ", 56, 3); gd.pain("TÊN LỚP", 69, 3); gd.pain("SỐ ĐT", 85, 3);
            gd.pain("╔═════════╦══════════╦═══════════════════╦═════════╦═══════════╦════════════╦═════════════════╗", 2, 2);
            gd.pain("╚═════════╩══════════╩═══════════════════╩═════════╩═══════════╩════════════╩═════════════════╝", 2, 45);
            for (int i = 3; i < 45; ++i)
            {
        ...
        public void suasv()
        {
            ...
            Console.Clear();
            List<SV> list = svbll.LSV();
            gd.ve(30, 2); Console.WriteLine("SỬA THÔNG TIN SINH VIÊN");
            string masv;
            gd.ve(15, 4); Console.Write("Nhập mã sinh viên cần sửa:");
            gd.ve(45, 4); masv = Console.ReadLine();
            int i = 0;
            ...
            if (i < list.Count)
            {
                ...
                gd.ve(15, 5); Console.Write("Nhập mã sinh viên mới:");
                gd.ve(45, 5); string msvm = Console.ReadLine();
                if (!string.IsNullOrEmpty(msvm)) s.Masv = msvm;
                gd.ve(15, 6); Console.Write("Nhập mã phòng mới:");
                gd.ve(45, 6); string mpm = Console.ReadLine();
                if (!string.IsNullOrEmpty(mpm)) s.Maph = mpm;
                gd.ve(15, 7); Console.Write("Nhập tên sv mới");
                gd.ve(45, 7); string ten = Console.ReadLine();
                if (!string.IsNullOrEmpty(ten)) s.Tensv = ten;
                gd.ve(15, 8); Console.WriteLine("giới tính mới");
                gd.ve(45, 8); string gtm = Console.ReadLine();
                if (!string.IsNullOrEmpty(gtm)) s.Gioitinh = gtm;
                gd.ve(15, 9); Console.WriteLine("địa chỉ mới");
                gd.ve(45, 9); string dcm = Console.ReadLine();
                 if (!string.IsNullOrEmpty(dcm)) s.Diachi = dcm;
                gd.ve(15, 10); Console.WriteLine("tên lớp mới");
                gd.ve(45, 10); string tlm = Console.ReadLine();
                if (!string.IsNullOrEmpty(tlm)) s.Tenlop = tlm;
                gd.ve(15, 11); Console.WriteLine("số điện thoại mới");
                gd.ve(45, 11);  string sdtm = Console.ReadLine();
                if (!string.IsNullOrEmpty(sdtm)) s.Sdt1 = sdtm;
                svbll.suattsv(s); 
            }
            else
            {
